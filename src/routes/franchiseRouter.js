...
franchiseRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    logger.log("info", "franchise", {
      event: "list_all_franchises",
...
franchiseRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.user.id === userId || req.user.isRole(Role.Admin)) {
      ...
      logger.log("info", "franchise", {
        event: "list_user_franchises",
        userId,
        count: result.length,
      });
    } else {
      logger.log("warn", "franchise", {
...
franchiseRouter.post(
  ...
  asyncHandler(async (req, res) => {
    if (!req.user.isRole(Role.Admin)) {
      logger.log("warn", "franchise", {
        event: "create_franchise_unauthorized",
        userId: req.user.id,
      });
...
franchiseRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    if (!req.user.isRole(Role.Admin)) {
      logger.log("warn", "franchise", {
        event: "delete_franchise_unauthorized",
        userId: req.user.id,
      });
...
franchiseRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      ...
      logger.log("warn", "franchise", {
        event: "create_store_unauthorized",
        franchiseId,
        userId: req.user.id,
      });
...
franchiseRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      ...
      logger.log("warn", "franchise", {
        event: "delete_store_unauthorized",
        franchiseId,
